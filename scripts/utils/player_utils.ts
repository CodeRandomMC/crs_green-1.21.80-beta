import { ContainerSlot, EquipmentSlot, Player } from "@minecraft/server";
...
export class PlayerUtils {
   ...
   */
  static getEquipmentSlot(player: Player, slot: EquipmentSlot): ContainerSlot | undefined {
    try {
      if (!player) {
      ...
      console.error(`Error in getEquipmentSlot for player ${player?.name || "Unknown"}: ${error}`);
      return undefined;
    }
  }
}
