export var CustomStateSet;
(function (CustomStateSet) {
    CustomStateSet["CROP_AGE"] = "crs_green:crop_age";
...
export class BlockStateUtils {
     ...
     * ```
     */
    static getState(block, state_name) {
        const value = block.permutation.getState(state_name);
     ...
     * ```
     */
    static getInt(block, state_name) {
     ...
     * ```
     */
    static getString(block, state_name) {
        const value = this.getState(block, state_name);
     ...
     * ```
     */
    static getBool(block, state_name) {
        const value = this.getState(block, state_name);
     ...
     * ```
     */
    static setState(block, state_name, value) {
        var _a;
        if (block.isValid) {
            try {
                if (typeof value === "number" && !Number.isInteger(value)) {
                ...
                block.setPermutation(block.permutation.withState(state_name, value));
                return true;
            }
            catch (error) {
                const errorMsg = error instanceof Error ? ((_a = error.message) !== null && _a !== void 0 ? _a : "Unknown error") : "Unknown error";
